...
public class PrtBulletinYear {
    ...
    public static String eleve(Context context, MEleve elv, MYear myear, int effectif){
        ...
        str_eleve = str_eleve.replace("$mention$", mention_10(myear.moy));
    ...
    public static String table_footer(Context context, MYear myear, MMoy1 moy1, MMoy2 moy2, MMoy3 moy3){
        String str_table_footer = Fil.rawFile2Str(context, R.raw.html_yearly_bulletin_table_footer);
        ...
        str_table_footer = str_table_footer.replace("$total_tri3$", myear.total_tri3);
        ...
        str_table_footer = str_table_footer.replace("$moy_tri3$", myear.moy_tri3);
        ...
        str_table_footer = str_table_footer.replace("$rang_tri3$", moy3 == null ? "0" : moy3.code_matiere);
    ...
    public static void save_bulletin(Context context, String html, String classe, String fname_pdf){
        try {
            ...
            File dir = new File(String.format("%s%s", root.getAbsolutePath(), "/bulletins-annuelle"));
            dir.mkdir();

            dir = new File(String.format("%s%s/%s", root.getAbsolutePath(), "/bulletins-annuelle", classe));
            dir.mkdir();

            dir = new File(String.format("%s%s%s/%s.pdf", root.getAbsolutePath(), "/bulletins-annuelle/", classe, fname_pdf));
