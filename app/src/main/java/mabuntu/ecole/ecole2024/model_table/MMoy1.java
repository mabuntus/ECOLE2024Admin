...
public class MMoy1 {
    ...
    private static final String fname = FILE.moy1;
    ...
    public static MMoy1 create_if_nexist(Context context, MEleve elv){
        MMoy1 mmoy = fromCodeElv(context, elv.code);
        if (mmoy != null)
            return mmoy;

        add(context, TCode.moy1(), elv.code, elv.classe, "code-matiere", "0.0", "0.0");
        return fromCodeElv(context, elv.code);
    }

    public static MMoy1[] fromCodeMatiere(Context context, String codeMatiere){
        return from_(context, niv_code_matiere, codeMatiere);
    }
    public static MMoy1[] fromClasse(Context context, String classe){
        return from_(context, niv_classe, classe);
    }
    public static MMoy1 fromCodeElv(Context context, String codeElv){
        MMoy1[] mmoys = from_(context, niv_code_eleve, codeElv);
        if (mmoys == null) return null;
        return mmoys[0];
    }
    private static MMoy1[] from_(Context context, int colNiv, String colVal){
        ArrayList<String[]> list = TDb.getAll(context, fname, colNiv, colVal);
        if (list == null || list.isEmpty()) return null;

        int size = list.size();
        MMoy1[] mMoys = new MMoy1[size];
    ...
    public static MMoy1 tab(String[] tab){
        ...
        return new MMoy1(code, code_eleve, classe, code_matiere, total, moy);
    }
}
