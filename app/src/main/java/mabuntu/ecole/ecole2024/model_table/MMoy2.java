...
public class MMoy2 {
    ...
    private static final String fname = FILE.moy2;
    ...
    public static MMoy2 create_if_nexist(Context context, MEleve elv){
        MMoy2 mmoy = fromCodeElv(context, elv.code);
        if (mmoy != null)
            return mmoy;

        add(context, TCode.moy2(), elv.code, elv.classe, "code-matiere", "0.0", "0.0");
        return fromCodeElv(context, elv.code);
    }

    public static MMoy2[] fromCodeMatiere(Context context, String codeMatiere){
        return from_(context, niv_code_matiere, codeMatiere);
    }
    public static MMoy2[] fromClasse(Context context, String classe){
        return from_(context, niv_classe, classe);
    }
    public static MMoy2 fromCodeElv(Context context, String codeElv){
        MMoy2[] mmoys = from_(context, niv_code_eleve, codeElv);
        if (mmoys == null) return null;
        return mmoys[0];
    }
    private static MMoy2[] from_(Context context, int colNiv, String colVal){
        ArrayList<String[]> list = TDb.getAll(context, fname, colNiv, colVal);
        if (list == null || list.isEmpty()) return null;

        int size = list.size();
        MMoy2[] mMoys = new MMoy2[size];
    ...
    public static MMoy2 tab(String[] tab){
        ...
        return new MMoy2(code, code_eleve, classe, code_matiere, total, moy);
    }
}
